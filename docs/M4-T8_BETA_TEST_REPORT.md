...
本次 Beta 测试共执行 8 个用例（BC-1 ~ BC-8）。其中 BC-8 在首轮测试中发现蓝牙耳机已连接但音频未走蓝牙的问题，经定位后修复并复测通过。其余用例均一次通过。测试过程中未出现 ANR、Crash 或阻塞性连接失败。
...
| BC-4 省电模式通话 | ✅ 通过 | 无 | 音质可接受，Neighbors 正常刷新 |
