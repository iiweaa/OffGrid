...
> 注：原 Auto 角色已在 M4-T5-FIX 中移除，仅保留手动 Group Owner / Client 两种角色。
...
- **Role** 下拉框：Group Owner / Client。
- **GO Config** 仅在 Role = Group Owner 时显示，允许用户自定义 Group Name 与 Passphrase；若留空则使用系统默认。
- **提示文案**：
  - 当检测到多台设备同时选择 Group Owner 时，提示「请确保仅一台设备作为 Group Owner」。
  - 该提示通过监听当前网络状态与角色配置判断。
...
| 多台设备同时选择 Group Owner | 请确保仅一台设备作为 Group Owner，其余设备选择 Client。 |
...
| 两台设备同时选择 Group Owner 导致无法互通 | 中 | 明确提示用户仅一台设备可作为 Group Owner，其余切换为 Client。 |
