...
> 输入：`docs/PRD.md` v1.1、`docs/ARCHITECTURE.md` v1.1
> 周期：26 周（约 6 个月）
...
|----------|------|
| 项目名称 | OffGrid |
| 项目目标 | 打造一款 MIT 开源的 Android 12+ 离线网状语音通讯应用 |
| 核心功能 | 全双工语音群聊、多跳中继、节点发现、相对方位/距离共享 |
| 目标用户 | 2-5 人徒步/骑行小队 |
...
| M3 | 第 7-12 周 | 6 周 | 多跳中继 + 自动组网 + 位置共享 | Alpha 版本 |
...
| M1-T2 | 调研并验证 Wi-Fi Direct 单跳直连可行性，记录 AP-STA 并发能力（目标机型 2-3 台） | Android Dev / Architect | 4d | M1-T1 | 两台手机成功建立 Wi-Fi Direct Group 并互相 ping 通；明确记录并发能力「支持/不支持」 |
...
- [x] AP-STA 并发能力记录在册，并明确 MVP 是否降级为单跳
...
| **周五** | M1-T2：测试结果记录与周报 | 直连测试记录表、并发能力结论、第 1 周周报 | 明确记录「单跳可用/并发不支持」结论 |

**第一周风险点：**
- 若目标机型不支持 AP-STA 并发，需立即调整架构假设（降级为单跳直连或更换测试机型）
...
### 3.3 M3：双机稳定 + 位置共享 + 多跳预备（第 7-12 周）

> 说明：当前目标机型（一加 11、华为/荣耀）实测不支持 Wi-Fi Direct AP-STA 并发，无法构成链式多跳。M3 修订为在当前两台设备上把双机体验做扎实，并保留多跳扩展结构；多跳中继本身推迟到 M4 或拿到支持并发机型后实施。

| ID | 任务 | 负责人 | 工期 | 依赖 | 验收标准 |
|----|------|--------|------|------|----------|
| M3-T1 | 运行时检测 AP-STA 并发能力并提示用户 | Android Dev | 2d | M2-T5 | 启动时显示「当前设备支持/不支持多跳中继」 |
| M3-T2 | Mesh 包格式 + NodeID + 基础序列化 | Android Dev / Architect | 3d | M2-T5 | 包结构符合架构文档，单测通过 |
| M3-T3 | 重构 LinkManager → NeighborTable | Android Dev | 3d | M3-T2 | 双机仍能互通，邻居表显示对端 NodeID、IP、LastSeen |
...
| M3-T8 | 双机 Alpha 集成测试与文档更新 | QA / PM | 3d | M3-T6, M3-T7 | 两台设备通话+位置共享 15 分钟无崩溃 |

**M3 里程碑验收（对应 PRD AC03/AC04/AC08 的部分内容）：**
- [ ] 两台设备稳定通话 + 位置共享 ≥ 15 分钟（实机测试待执行）
- [x] 方位界面显示队友相对方位与距离
- [x] 代码保留多跳扩展结构（TTL、NodeID、转发占位）
- [x] 明确提示用户：当前机型是否支持多跳中继
...
| M4-T8 | Beta 版本测试（多机型、多场景） | QA / PM | 6d | M4-T1~T7 | Beta 测试报告，无 P0/P1 Bug |
...
### 3.5 M5：户外实测、Bug 修复、社区发布（第 23-26 周）

| ID | 任务 | 负责人 | 工期 | 依赖 | 验收标准 |
|----|------|--------|------|------|----------|
| M5-T1 | 制定户外测试方案（场地、路线、设备、数据记录） | PM / QA | 3d | M4-T8 | 测试方案文档化 |
| M5-T2 | 执行山地/公路户外实测 | QA / PM / Android Dev | 5d | M5-T1 | 收集距离、延迟、续航、稳定性数据 |
| M5-T3 | 根据实测结果修复 Bug 与优化性能 | Android Dev | 7d | M5-T2 | 实测 P0/P1 Bug 清零 |
...
| PRD 功能 | 主要任务 | 验收标准 |
|----------|----------|----------|
| F01 网状语音 | M2-T3, M2-T6, M3-T2, M3-T4 | AC01, AC02, AC03 |
| F02 多跳中继 | M3-T2, M3-T4 | AC03, AC08 |
| F03 节点发现 | M3-T3 | AC01 |
| F04 位置共享 | M3-T5, M3-T6 | AC04 |
| F05 音频 I/O | M2-T3, M4-T2, M4-T3 | AC06 |
| F06 开源发布 | M2-T7, M4-T7, M5-T4 | AC07 |
| F08 拓扑可视化 | M3-T7 | - |
| F09 自动重连 | M4-T1 | AC05 |
...
3. **启动 M1-T1 与 M1-T2**：环境搭建 + AP-STA 并发验证
4. **每周五进行进度 Review**

---

*本项目计划由管师基于 PRD v1.1 与架构设计 v1.1 制定，待用户确认后执行。*
